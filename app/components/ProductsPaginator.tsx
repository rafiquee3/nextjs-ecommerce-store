...
import { ProductsTable } from "./ProductCard/ProductsTable";
...
export default function ProductsPaginator({initialProducts, totalCount, initialFilterParams, isAdmin = false}: ProductsPaginatorProps) {
    ...
    const updateURL = useCallback((key: string, value: string | number) => {
        ...
        router.push(`${isAdmin ? '/admin' : ''}/products?${currentParams.toString()}`);
    }, [router, searchParams]);
    ...
    return (
        <div className="w-screen min-w-[375px] md:w-full items-center flex flex-col md:gap-2">
            <div className="w-full md:w-fit">
                <div className="flex w-full md:w-[570px] text-sm">
                    <div className="relative w-2/5">
                        <select 
                            className="safari-reset truncate border-r-[1px] border-gray-400 md:rounded-tl-lg py-1 px-6 w-full cursor-pointer border-r-0"
                            id='cat_select'
                            onChange={handleCategoryChange} 
                            defaultValue={initialFilterParams.category}
                        >
                    ...
                    <div className="relative w-2/5">
                        <select 
                            className="safari-reset truncate border-r-[1px] border-gray-400 py-1 pl-3 pr-6 w-full cursor-pointer border-r-0"
                            id='sort_select'
            ...
            </div>
            {
                ...
                :
                <div className="product-list grid grid-cols-2 md:grid-cols-3 gap-6">
                    {initialProducts.map(product => (
                        <div key={product.id} className="w-[170px] md:w-[220px]">
