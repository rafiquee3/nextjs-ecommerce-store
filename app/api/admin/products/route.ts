...
export async function POST(request: Request) {
    ...
    try {
        ...
        const {name} = newProductData;

        const product = await findProduct(name);
        if (product.isFound) {
            return NextResponse.json({ 
                message: "The provided product has already been added to the database.", 
            }, { status: 409 });
        }
        ...
        const newId = products.length > 0 ? Math.max(...products.map((p: any) => p.id)) + 1 : 100;
        ...
        return NextResponse.json({ 
            message: "Product added successfully (Database simulated).", 
            product: newProduct 
        }, { status: 201 });
...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (category !== 'all') filteredProducts = products.filter(p => p.category === category);
